...
public class IRBuilder implements ASTVisitor {
    ...
    public IRBuilder(GlobalScope gScope) {
        ...
        program.IRBuiltinFunc.add(new IRFuncDeclare("string__length", CharStar, Int));
        program.IRBuiltinFunc.add(new IRFuncDeclare("string__substring", CharStar, CharStar, Int, Int));
    ...
    public void declareClass(DefineClassNode it) {
        ...
        for (var nxt: it.functions) {
            declareFunc(nxt, it.className);
        }
    }

    public void declareFunc(DefineFunctionNode it, String className) {
        Function func;
        if (className != null) {
            func = new Function(className + "__" + it.funcName, getIRType(it.type));
            IRStructType struct = program.IRclassMap.get(className);
            IRPtrType ptr = new IRPtrType(struct);
            IRRegister thisReg = new IRRegister("this", ptr);
            func.params.add(thisReg);
        }
        else {
            func = new Function(it.funcName, getIRType(it.type));
        }
    ...
    public Entity allocaNew(IRBaseType type, int index, ArrayList<ExprNode> siz) {
        ...
        curBlock.addInst(new IRBinaryOp(curBlock, tmp_reg, new IRIntType(32), "mul", new IRIntConst(nxt_type.size / 8), getVal(siz.get(index), false)));
        ...
        BasicBlock forCond = new BasicBlock("new_for.cond", inFunc, loopDepth + 1);
        BasicBlock forInc = new BasicBlock("new_for.inc", inFunc, loopDepth + 1);
        BasicBlock forBody = new BasicBlock("new_for.body", inFunc, loopDepth + 1);
        BasicBlock forEnd = new BasicBlock("new_for.end", inFunc, loopDepth);
    ...
    public void visit(NewExprNode it) {
        if (it.type.dim != 0) {
            ...
            callInst.args.add(new IRIntConst(classType.type.size / 8));
            curBlock.addInst(callInst);
    ...
    public void visit(ArrayExprNode it) {
        ...
        IRGetElementPtr getPtrinst = new IRGetElementPtr(curBlock, getVal(it.array, false), getIRType(it.type), res);
        getPtrinst.idx.add(getVal(it.index, false));
        curBlock.addInst(getPtrinst);
        it.addr = res;
    }
    ...
    public void visit(SingleExprNode it) {
        ...
        if (it.op.equals("+")) {
            ...
            IRRegister res = new IRRegister("single_expr" , new IRIntType(8));
    ...
    public void visit(BinaryExprNode it) {
        ...
        if (!it.op.equals("&&") && !it.op.equals("||")) {
                ...
                if (it.lop.isConst && it.rop.isConst) {
                    switch (it.op) {
                        case "+":
                            ...
                            break;
                        case "==":
                            it.entity = new IRBoolConst(((IRStringConst) it.lop.entity).val.equals(((IRStringConst) it.rop.entity).val));
                            break;
                        case "!=":
                            it.entity = new IRBoolConst(!((IRStringConst) it.lop.entity).val.equals(((IRStringConst) it.rop.entity).val));
                            break;
                        case "<":
                            it.entity = new IRBoolConst(((IRStringConst) it.lop.entity).val.compareTo(((IRStringConst) it.rop.entity).val) < 0);
                            break;
                        case ">":
                            it.entity = new IRBoolConst(((IRStringConst) it.lop.entity).val.compareTo(((IRStringConst) it.rop.entity).val) > 0);
                            break;
            ...
            if (flag) {
                ...
                curBlock.addInst(new IRIcmp(curBlock, res, op, getIRType(it.lop.type), getVal(it.lop, false), getVal(it.rop, false)));
                it.entity = res;
            }
            ...
            BasicBlock rightBranch = new BasicBlock("binary_op.rhs", inFunc, loopDepth);
            BasicBlock shortCircuitbranch = new BasicBlock("binary_op.short", inFunc, loopDepth);
            BasicBlock endBinaryOp = new BasicBlock("binary_op.end", inFunc, loopDepth);
    ...
    public void visit(TernaryExprNode it) {
        ...
        BasicBlock prev = curBlock;
        BasicBlock trueBranch = new BasicBlock("ternary.then", inFunc, loopDepth);
        BasicBlock falseBranch = new BasicBlock("ternary.else", inFunc, loopDepth);
        BasicBlock endTernary = new BasicBlock("ternary.end", inFunc, loopDepth);
    ...
    public void visit(AtomExprNode it) {
        if (it.isIdentifier) {
            ...
            if (curScope.inClass != null && it.addr == null) {
    ...
    public void visit(IfStmtNode it) {
        it.condition.accept(this);
        BasicBlock prev = curBlock;
        BasicBlock trueBranch = new BasicBlock("if.then", inFunc, loopDepth);
        BasicBlock falseBranch = new BasicBlock("if.else", inFunc, loopDepth);
        BasicBlock endIf = new BasicBlock("if.end", inFunc, loopDepth);
    ...
    public void visit(ForStmtNode it) {
        curScope = new Scope(curScope);
        BasicBlock prev = curBlock;
        BasicBlock forCond = new BasicBlock("for.cond", inFunc, loopDepth + 1);
        BasicBlock forInc = new BasicBlock("for.inc", inFunc, loopDepth + 1);
        BasicBlock forBody = new BasicBlock("for.body", inFunc, loopDepth + 1);
        BasicBlock forEnd = new BasicBlock("for.end", inFunc, loopDepth);
        curScope.breakNxt = forEnd;
        curScope.continueNxt = forInc;
        if (it.defInit != null) {
    ...
    public void visit(WhileStmtNode it) {
        curScope = new Scope(curScope);
        BasicBlock prev = curBlock;
        BasicBlock whileCond = new BasicBlock("while.cond", inFunc, loopDepth + 1);
        BasicBlock whileBody = new BasicBlock("while.body", inFunc, loopDepth + 1);
        BasicBlock whileEnd = new BasicBlock("while.end", inFunc, loopDepth + 1);
        curScope.breakNxt = whileEnd;
        curScope.continueNxt = whileCond;
    ...
    public void visit(BreakStmtNode it) {
        curBlock.addInst(new IRJump(curBlock, curScope.breakNxt.label));
    }

    @Override
    public void visit(ContinueStmtNode it) {
        curBlock.addInst(new IRJump(curBlock, curScope.continueNxt.label));
    }
    ...
    public void visit(ParamsListNode it) {
        ...
        for (int i = 0; i < inFunc.params.size(); i++) {
            ...
            curScope.entityMap.put(it.identifiers.get(inClass ? i - 1 : i), addr);
        }
    }
    ...
    public void visit(DefineConstructFunctionNode it) {
        ...
        curBlock = new BasicBlock("entry", inFunc, loopDepth, false);
        ...
        curBlock = new BasicBlock("return", inFunc, loopDepth, false);
    ...
    public void visit(DefineFunctionNode it) {
        ...
        if (curScope.inClass != null) {
            ...
            func = gScope.IRfuncMap.get(it.funcName);
        }
        inFunc = func;
        curBlock = new BasicBlock("entry", inFunc, loopDepth, false);
        inFunc.blocks.add(curBlock);
        ...
        else if (curScope.inClass != null){
            IRStructType struct = program.IRclassMap.get(curScope.inClass.className);
            IRPtrType ptr = new IRPtrType(struct);
            IRRegister thisAddr = new IRRegister("this.addr", new IRPtrType(ptr));
            IRRegister thisReg = inFunc.params.get(0);
            inFunc.thisPtr = thisAddr;
            curBlock.addInst(new IRAlloca(curBlock, thisAddr.type, thisAddr));
            curBlock.addInst(new IRStore(curBlock, thisAddr, thisReg));
        }
    ...
    public void visit(RootNode it) {
        for (var nxt: it.Defs) {
            ...
            if (nxt instanceof DefineFunctionNode) {
                declareFunc((DefineFunctionNode) nxt, null);
            }
        }
        for (var nxt: it.Defs) {
            if (nxt instanceof DefineClassNode) {
